...
const (
	appKeyScretKeyTypeName = "AppKeyScretKeyAuth"
)

type appKeyScretKeyAuth struct {
	next         http.Handler
	headerField  string
	removeHeader bool
	name         string
...
func NewAppKeyScretKeyAuth(ctx context.Context, next http.Handler, authConfig dynamic.AppKeyAuth, name string) (http.Handler, error) {
...
func (b *appKeyScretKeyAuth) GetTracingInformation() (string, ext.SpanKindEnum) {
	return b.name, tracing.SpanKindNoneEnum
}

func (a *appKeyScretKeyAuth) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
...
func (a *appKeyScretKeyAuth) RequireAuth(w http.ResponseWriter, r *http.Request) {
...
func (a *appKeyScretKeyAuth) verify(appKey string, apiName string ) bool {
	//todo
	if appKey =="" {
		return false
	}
